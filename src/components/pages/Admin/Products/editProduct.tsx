...
const EditProduct: React.FC<Props> = ({ productId }) => {
  const product = {
    ...
    maxQuantity: 10,
    availableDelivery: [
      { pincode: "474011", estimateDeliveryInDays: 2 },
      {
        pincode: "474006",
        estimateDeliveryInDays: 4,
      },
    ],
    availableColor: [
      {
    ...
    productSpecification: {
      ...
      specification: "Apple Iphone 16 pro max is coming with 256 GB storage",
    },
  };
      ...
      <div className="border border-gray-200 shadow-lg rounded overflow-hidden w-full">
        ...
        <div className="px-2 py-3">
          ...
          <FormRow
            ...
            isImage
            value={banner}
            onChange={(val) => updateState({ banner: val })}
          />
        </div>
      </div>
    );
  ...
  const renderAvailable = () => {
    const { availableDelivery, availableColor, productImages } = state;
  ...
  useEffect(() => {
    ...
    if (product.productSpecification.availableSize?.length > 0) {
